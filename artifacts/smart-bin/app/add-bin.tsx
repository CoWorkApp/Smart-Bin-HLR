...
export default function AddBin() {
  const {
    ...
    qrCode: prefilledQrCode,
  } = useLocalSearchParams<{ locationId?: string; qrCode?: string }>();

  const colors = useColors();
  const insets = useSafeAreaInsets();
  const { addBin, getGroupLocations, getLocationPath } = useApp();
  ...
  function handleSave() {
    if (!name.trim()) return;
    addBin(name.trim(), selectedLocationId || null, qrCode.trim() || undefined);
  ...
  return (
    ...
    >
      <Text style={[styles.title, { color: colors.foreground }]}>Add Bin</Text>
      <Text style={[styles.subtitle, { color: colors.mutedForeground }]}>
        Bins are containers within a location that hold your items.
      </Text>
      ...
      >
        <Feather
          name="map-pin"
          size={16}
          color={selectedLocation ? colors.primary : colors.mutedForeground}
        />
        <Text
          style={[
      ...
      {qrCode ? (
        ...
        <Text style={[styles.hint, { color: colors.mutedForeground }]}>
          Assign a physical QR code to instantly access this bin by scanning.
        </Text>
      )}
