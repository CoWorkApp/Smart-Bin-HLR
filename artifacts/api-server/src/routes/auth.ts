...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const { code, code_verifier, redirect_uri, state } = req.body as {
      code?: string;
    ...
    try {
      ...
      const claims = tokens.claims();
      if (!claims?.sub) {
        res.status(401).json({ error: "No identity in Google token" });
        return;
      }

      const googleClaims: Record<string, unknown> = {
        sub: `google:${claims.sub}`,
        email: claims.email ?? null,
        first_name: (claims.given_name as string) ?? null,
        last_name: (claims.family_name as string) ?? null,
      ...
      const sessionData: SessionData = {
        ...
        expires_at: tokens.expiresIn
          ? now + (tokens.expiresIn as unknown as number)
          : now + 3600,
      };
      ...
      req.log.error({ err }, "Google token exchange error");
